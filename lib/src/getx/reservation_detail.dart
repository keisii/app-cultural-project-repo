...
class DataSource {
  Future<Reservation> createReservation(String? boardId, String name, DateTime reservationDate, int count, String phoneNum) async {
    try {
      final url = Uri.parse("http://192.168.100.221:9000/reserve-service/reserve"); // 각자 본인 컴퓨터의 localhost로 설정
  ...
  Future<List<Reservation>> findReservation(String name, String phoneNum) async {
    try {
      final url = Uri.parse("http://192.168.100.221:9000/reserve-service/${name}/${phoneNum}"); // 각자 본인 컴퓨터의 localhost로 설정
      print(url);
  ...
  Future<void> deleteReservation(List<dynamic> list, String name, String phoneNum) async {
    try {
      final url = Uri.parse("http://192.168.100.221:9000/reserve-service");
